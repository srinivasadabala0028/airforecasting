...
if geocoding_response.status_code == 200:
    ...
    if geocoding_data:
        ...
        # Step 2: Specify the start and end dates in UTC format
        start_date_utc = datetime(2020, 11, 27, 0, 0, 0)  # November 27, 2020, 00:00:00 UTC
        start_timestamp_utc = int(start_date_utc.timestamp())

# Calculate the end date as today's date in UTC
        end_date_utc = datetime.utcnow()  # Current date and time in UTC
        end_timestamp_utc = int(end_date_utc.timestamp())
        # Step 3: Historical Air Pollution API to get historical air quality data
        historical_air_pollution_url = f"http://api.openweathermap.org/data/2.5/air_pollution/history?lat={latitude}&lon={longitude}&start={start_date_utc}&end={end_date_utc}&appid={api_key}"
